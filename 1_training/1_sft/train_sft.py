...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="64 (not 2) by default: LoRA's memory footprint (frozen backbone + a "
        ...
        "for an effective batch size of 64 with no accumulation.",
    )
    parser.add_argument(
        "--grad-accum",
        type=int,
        ...
        "--per-device-batch-size if 64 turns out to be too large for the GPU's memory.",
    )
    parser.add_argument(
        "--max-seq-length",
        type=int,
