...
| `templates/inference/vllm-serve-checkpoint-fp8-dflash.yaml` | Template (substitute `__NAME__`/`__RUN_ID__` — see its header) serving a specific MLflow-tracked run's model artifact with weights and KV-cache both in FP8, plus DFlash speculative decoding (`z-lab/Qwen3-8B-DFlash-b16` drafter, 4 candidate tokens) — everything baked in, not routed through `vllm-serve-checkpoint.yaml`'s `__VLLM_EXTRA_ARGS__` (the drafter's model id collides with `sed`'s own delimiter). DFlash, not MTP: MTP needs a checkpoint with a trained MTP head, which this project's plain dense Qwen3-8B fine-tunes don't have (confirmed to fail at load); DFlash's drafter is a separate standalone model verified against the target via rejection sampling, so it needs no special target-side training. |
| `templates/inference/run-vllm-bench-quantization-suite.sh` | Runs the full `vllm bench serve` sweep against four serving configs of the same checkpoint in one command — bf16, FP8 weights, FP8 weights+KV-cache, and FP8 weights+KV-cache+DFlash — each labelled and logged to MLflow as its own run (`SFT_RUN_ID` required; `NUM_PROMPTS`/`CONCURRENCIES` overridable). Manages its own port-forwarding to each Deployment in turn; requires `kubectl -n fin-agent port-forward svc/mlflow 5000:5000` running separately. |
...
| `templates/inference/bfcl-eval-mlflow-checkpoint-job.yaml` | Template (substitute `__NAME__`/`__RUN_ID__`/`__TEST_CATEGORIES__` — see its header) for a BFCL evaluation against a specific MLflow-tracked run, with its own bundled vLLM Deployment that pulls the model via `mlflow.artifacts.download_artifacts` (same mechanism as `vllm-serve-checkpoint.yaml`, minus any quantization/speculative-decoding flags — this is a correctness eval, not a serving config). `__TEST_CATEGORIES__` is a space-separated list looped over in one MLflow run, so a cheap smoke pass can still populate both Non-Live and Live AST accuracy. Rendered and applied by `run-bfcl-eval-run-ids-suite.sh`. |
| `templates/inference/bfcl-eval-baseline-categories-job.yaml` | Same `__TEST_CATEGORIES__` loop mechanism as above, but Job-only, against `vllm-qwen3-8b.yaml`'s Deployment — the baseline's leg of `run-bfcl-eval-run-ids-suite.sh`. Kept separate from `bfcl-eval-job.yaml`/`run-bfcl-eval.sh` (untouched, single-category) so existing usage isn't disturbed. |
| `templates/inference/run-bfcl-eval-run-ids-suite.sh` | The run_id analogue of `run-bfcl-eval-suite.sh`: evaluates the baseline plus two specific MLflow-tracked runs (`SFT_RUN_ID`/`GRPO_RUN_ID` env vars, defaulted) rather than sweeping local checkpoints. Same smoke-by-default/`FULL_SCALE=1` convention, and every run (smoke or real) logs both `bfcl_non_live_ast_accuracy` and `bfcl_live_ast_accuracy` for all three models. |
